...
async def get_last_research():
    ...
    Results_Summary = os.path.join(os.path.dirname(__file__), 'data/Results_Summary')
    #print(Results_Summary)
    # List to store JSON content from each file
    json_responses = []

    # Iterate over all files in the "data" directory
    for file_name in os.listdir(Results_Summary):
        print(file_name)
        if file_name.endswith('.json'):
            # Construct the file path
            json_file_path = os.path.join(Results_Summary, file_name)
            ...
            json_responses.append(JSONResponse(content=json_result))
